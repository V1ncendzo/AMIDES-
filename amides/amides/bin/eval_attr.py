...
def create_rules_evasions_dict(rules_data):
    ...
    for rule_data in rules_data.rule_datasets.values():
        if malicious_samples_type == "matches":
            target_events = rule_data.matches
        else:
            target_events = rule_data.evasions
        ...
        rules_evasions_dict[rule_data.name] = list(
            extractor.extract_commandline(target_events.data)
        )
...
def create_evasions_rules_list(rules_evasions_dict):
    # CHANGED: return list of (evasion, rule) tuples to allow duplicates
    evasions_rules_list = []
    for rule, evasions in rules_evasions_dict.items():
        for evasion in evasions:
            evasions_rules_list.append((evasion, rule))

    return evasions_rules_list
...
def process_samples(evasions_rules_list):
    # CHANGED: input is matching list of tuples
    samples = [item[0] for item in evasions_rules_list]
    rules = [item[1] for item in evasions_rules_list]
    ...
    for i, sample in enumerate(samples):
        ...
        # Use the stored rule for this index
        true_rule = rules[i]
        results.append((true_rule, sorted_sample_results))
...
def evaluate_rule_attribution():
    ...
    # CHANGED: use list to preserve duplicates
    evasions_rule_list = create_evasions_rules_list(rules_evasions_dict)

    eval_rslt = RuleAttributionEvaluationResult(
        num_rules=len(rule_attributor.keys()), timestamp=""
    )

    results = process_samples(evasions_rule_list)
...
def main():
    ...
    parser.add_argument(
        ...
        default="evasions",
        help="Type of malicious samples used for evaluation",
    )
