...
def validation_samples(malicious_samples: list[str]):
    ...
    # Read benign samples into a list first (simplifies logic vs generator for normalization)
    for sample in open(benign_samples_path, "r", encoding="utf-8"):
        benign_samples.append(sample.rstrip("\n"))
        
    if normalize_benign_samples:
        import json
        # Normalized returns a list of token strings from list of json strings
        benign_samples = normalize(benign_samples)
...
def main():
    ...
    parser.add_argument(
        "--normalize",
        action="store_true",
        help="Normalize benign samples before validation",
    )
    parser.add_argument(
        "--events-dir",
        type=str,
