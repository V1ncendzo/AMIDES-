...
def prepare_training_data(
    ...
    benign_samples_or_factory = prepare_benign_samples_for_fitting()

    def get_samples():
        if callable(benign_samples_or_factory):
            return benign_samples_or_factory()
        return benign_samples_or_factory

    feature_vectors = vectorizer.fit_transform(
        train_samples(get_samples(), tainted_samples, malicious_samples)
    )

    labels = create_labels_array(
        count_benign_samples(get_samples()),
        len(tainted_samples),
